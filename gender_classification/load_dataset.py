...
imdb_mat = 'D:/dataset/imdb/imdb_crop/imdb_crop/imdb.mat'
wiki_mat = 'D:/dataset/wiki/wiki_crop/wiki_crop/wiki.mat'
...
for file in wiki_path:
    ...
    wiki_dob.append('-'.join(temp))

#print(imdb_dob, wiki_dob)
imdb_age = []
wiki_age = []

for i in range(len(imdb_dob)):
    try:
        d1 = date.datetime.strptime(imdb_dob[i], '%Y-%m-%d')
